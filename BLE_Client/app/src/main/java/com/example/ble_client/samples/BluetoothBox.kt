...
import com.example.ble_client.base.MissingFeatureText
import com.example.ble_client.base.PermissionBox

@Composable
fun BluetoothBox(
    extraPermissions: Set<String> = emptySet(),
    content: @Composable (BluetoothAdapter) -> Unit
) {
    ...
    PermissionBox(
        ...
        // Check to see if BLE feature is available
...
@Composable
fun BluetoothDisabledScreen(onEnable: () -> Unit) {
    val result =
    ...
    Column(
        ...
        Button(onClick = { result.launch(Intent(BluetoothAdapter.ACTION_REQUEST_ENABLE)) }) {
            Text(text = "Enable")
        }
    }
}