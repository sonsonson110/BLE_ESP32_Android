...
fun PermissionBox(
    ...
    val permissionState = rememberMultiplePermissionsState(permissions = permissions) { map ->
        val rejectPermissions = map.filterValues { !it }.keys
        errorText = if (rejectPermissions.none { it in requiredPermission }) {
            ""
        } else {
            "${rejectPermissions.joinToString()} required for the app to run properly"
        }
    }
    val allRequiredPermissionsGranted =
        permissionState.revokedPermissions.none { it.permission in requiredPermission }
    ...
    Box(
        ...
        if (allRequiredPermissionsGranted) {
            onGranted(permissionState.permissions
                .filter { it.status.isGranted }
                .map { it.permission })
        } else {
            PermissionScreen(permissionState, description, errorText)
        }
    }
}
